...
export default function HorariosBachilleratoClient({ teachers, subjects, sections, slots, scheduleOptions: initialOptions, gradeHours = [], error, initialGrade, initialSection }) {
  ...
  // Honor ?grade=X&section=Y URL params if they match a real section, otherwise fall back to first
  ...
  // ── Schedule options (loaded from DB, each has a real DB id) ─────────────
  ...
  // Keep options in sync when server data refreshes
  useEffect(() => {
  ...
  const createNewOption = async () => {
    ...
    try {
      ...
      // Refresh server data so the new option appears with its DB id
      startTransition(() => { router.refresh() })
      // Optimistically switch to the new option
  ...
  // Mark an option as principal in the DB
  const handleSetPrincipal = async (optId) => {
    ...
    try {
      ...
      // Update local flags optimistically
  ...
  const confirmDeleteOption = async () => {
    ...
    try {
      ...
      // If deleting principal, mark first remaining as principal locally
  ...
  // All options persist to DB — always pass option_id
  ...
  const handleExportPDF = async () => {
    ...
    pdf.text(`${activeOption?.label ?? 'Horario'} - ${selectedGrade}° ${selectedSection}`, margin, 14)
    ...
    pdf.save(`horario-${selectedGrade}${selectedSection}.pdf`)
  }
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden">
      ...
      <div className="bg-primary border-b border-primary-dark px-6 py-3 flex items-center justify-between shrink-0 shadow-card z-10">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {/* ── Schedule options dropdown trigger ────────────────────────── */}
